...
* https://github.com/apache/airflow[Apache Airflow] - A platform to programmatically author, schedule, and monitor workflows. Provides platform to build and manage DAGs (Directed Acyclic Graphs) for ETL pipelines, supporting integrations with cloud services, databases, and messaging systems. Provides a rich UI for tracking task execution, retry logic, and pipeline dependencies.
